...
**사용자는 파일명을 말하지 않는다.** 아래 표는 시작점이다. 읽어보고 연관 파일이 더 있으면 추가로 읽어도 된다.
...
## 작업 순서 (매 요청 시 따를 것)
1. 위 라우팅 표에서 관련 파일 파악
2. 해당 파일 읽기 (복합 기능이면 연관 파일도 읽기)
3. 수정
...
- **auto 클래스**: 자동채우기로 입력된 값 표시, 사용자가 편집하면 제거됨
- **fp(cid, pid)**: 패널 input값 → 리포트카드 innerText 단방향 복사
...
| 작업 유형 | 읽을 파일 |
|---|---|
| UI 기능 → 파일 상세 매핑, 자연어 예시 | `docs/feature-map.md` |
| UI 구조·상태 객체 G, IndexedDB 키 | `docs/ui-state.md` |
